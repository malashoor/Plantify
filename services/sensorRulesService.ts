...
export async function getSensorRules(plantId?: string): Promise<SensorRule[]> {
  let query = supabase.from('sensor_rules').select('*').order('created_at', { ascending: false });
...
export async function getSensorRule(id: string): Promise<SensorRule> {
  const { data, error } = await supabase.from('sensor_rules').select('*').eq('id', id).single();
...
export async function createSensorRule(input: CreateSensorRuleInput): Promise<SensorRule> {
  const { data, error } = await supabase.from('sensor_rules').insert([input]).select().single();
...
export async function deleteSensorRule(id: string): Promise<void> {
  const { error } = await supabase.from('sensor_rules').delete().eq('id', id);
