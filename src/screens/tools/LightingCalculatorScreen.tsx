...
export default function LightingCalculatorScreen() {
  ...
  const handleDistanceChange = useCallback(
    (value: number) => {
      ...
      if (isScreenReaderEnabled) {
        announceChange(
          t('lighting.accessibility.distance_changed', {
            distance: value,
  ...
  const handleSaveSetup = useCallback(async () => {
    ...
    try {
      ...
      announceChange(
        t('lighting.accessibility.setup_saved', {
          name: setupName,
        })
      );
      setSetupName('');
        ...
        >
          ...
          {selectedCrop && selectedStage && (
            ...
            >
              ...
              {commonLEDs.map(led => (
                <TouchableOpacity
                  key={led.id}
                  style={[styles.ledCard, selectedLED?.id === led.id && styles.ledCardSelected]}
                ...
                >
                  ...
                  <View style={styles.ledMetrics}>
                    <Text style={styles.metricText}>PPFD: {led.ppfd} μmol/m²/s</Text>
                    <Text style={styles.metricText}>Coverage: {led.coverage}m²</Text>
                  </View>
                </TouchableOpacity>
              ))}
            </View>
          ...
          {calculations && (
            ...
            >
              ...
              >
                <Text style={styles.resultTitle}>{t('lighting.daily_light_integral')}</Text>
                <Text style={styles.resultValue}>{calculations.dli.toFixed(1)} mol/m²/day</Text>

                <Text style={styles.resultTitle}>{t('lighting.power_consumption')}</Text>
                <Text style={styles.resultValue}>
                  {calculations.powerConsumption.toFixed(1)} kWh/day
                </Text>

                <Text style={styles.resultTitle}>{t('lighting.estimated_cost')}</Text>
                <Text style={styles.resultValue}>${calculations.estimatedCost.toFixed(2)}/day</Text>
              </View>
              ...
              <View style={styles.saveSetup}>
                ...
                >
                  <Text style={styles.saveButtonText}>{t('lighting.save_setup')}</Text>
                </TouchableOpacity>
              </View>
            </View>
          )}
