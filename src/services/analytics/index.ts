...
class AnalyticsService {
  ...
  async trackScreen(screenName: string, properties: AnalyticsProperties = {}): Promise<void> {
    if (!this.initialized || this.isOptedOut) return;
  ...
  // Revenue tracking
  async trackRevenue(amount: number, properties: AnalyticsProperties = {}): Promise<void> {
    if (!this.userId) return;
