...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    CREATE POLICY "Admins can read all user activities"
      ...
      FOR SELECT
      TO authenticated
      USING ((auth.jwt() ->> 'role')::text IN ('super_admin', 'support_admin'));
  END IF;
  ...
  IF NOT EXISTS (
    ...
    CREATE POLICY "Content moderators can manage reports"
      ...
      FOR ALL
      TO authenticated
      USING ((auth.jwt() ->> 'role')::text IN ('super_admin', 'content_moderator'));
  END IF;
  ...
  IF NOT EXISTS (
    ...
    CREATE POLICY "Analytics viewers can read analytics"
      ...
      FOR SELECT
      TO authenticated
      USING ((auth.jwt() ->> 'role')::text IN ('super_admin', 'analytics_viewer'));
  END IF;
...
CREATE INDEX IF NOT EXISTS idx_content_reports_status_timestamp ON content_reports(status, timestamp);
CREATE INDEX IF NOT EXISTS idx_user_bans_user_id_status ON user_bans(user_id, lifted_at NULLS FIRST);
CREATE INDEX IF NOT EXISTS idx_analytics_events_event_timestamp ON analytics_events(event, timestamp);

-- Add constraints for data integrity
ALTER TABLE content_reports ADD CONSTRAINT valid_content_type 
  CHECK (content_type IN ('plant', 'comment', 'profile'));

ALTER TABLE content_reports ADD CONSTRAINT valid_status 
  CHECK (status IN ('pending', 'reviewed', 'resolved'));

-- Add cascade delete for user data cleanup
...
ALTER TABLE content_reports
  DROP CONSTRAINT IF EXISTS content_reports_user_id_fkey,
