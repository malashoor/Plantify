...
export default function SeedDetailScreen() {
  ...
  const { fetchGuideBySpecies, getStageInstructions, getNextStage, getStageDuration } =
    useGrowthGuide();
  ...
  const handleScheduleWatering = async (date: Date) => {
    ...
    try {
      await scheduleWatering({
        ...
        date,
        amount:
          growingMethod.type === 'hydroponic' && growingMethod.nutrientSchedule
    ...
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          ...
          <Text style={[styles.species, { color: theme.colors.textSecondary }]}>
            {seed.species}
          </Text>
          {seed.variety && (
