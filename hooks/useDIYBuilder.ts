...
export const useDIYBuilder = () => {
  ...
  // Select system type
  const selectSystemType = useCallback((systemType: SystemType) => {
    setState(prev => ({ 
      ...prev, 
    ...
    if (Platform.OS !== 'web') {
      Speech.speak(
        t('diy.voice.systemSelected', { system: systemType.name }),
        { language: locale }
      );
    }
  }, [t, locale]);
  ...
  const calculateMaterials = useCallback((inputs: CalculationInput): CalculationResult => {
    ...
    try {
      ...
      totalCost = materials.reduce((sum, material) => {
        ...
        return sum + (baseMaterial ? baseMaterial.estimatedCost * material.calculatedQuantity : 0);
      }, 0);
  ...
  const generateMaterialsList = useCallback(async (
  ...
  const exportMaterialsList = useCallback(async (
    ...
    try {
      ...
      if (Platform.OS !== 'web') {
        Speech.speak(
          t('diy.voice.exportComplete', { format: format.toUpperCase() }),
          { language: locale }
        );
      }
    } catch (error) {
  ...
  // Start new build
  const startNewBuild = useCallback(async (
    templateId: string,
    buildName: string,
    ...
    try {
      ...
      if (Platform.OS !== 'web') {
        Speech.speak(
          t('diy.voice.buildStarted', { name: buildName }),
          { language: locale }
        );
      }
    } catch (error) {
  ...
  }, [calculateMaterials, state.isOffline, saveCurrentBuild, saveBuildToSupabase, generateMaterialsList, t, locale]);

  // Complete build step
  const completeStep = useCallback(async (stepId: string, notes?: string, images?: UserBuildImage[]) => {
    ...
    const recommendations: PumpRecommendation[] = [
      {
        ...
        reason: flowRate > 400 ? 'Flow rate too high' : headHeight > 8 ? 'Head height too high' : undefined
      },
      {
        ...
        reason: flowRate > 633 ? 'Flow rate too high' : headHeight > 10 ? 'Head height too high' : undefined
      }
    ];
  ...
  const getSystemTypes = useCallback((): SystemType[] => {
    return SYSTEM_TYPES.map(system => ({
      ...
      description: locale === 'ar' && system.descriptionAr ? system.descriptionAr : system.description,
      advantages: locale === 'ar' && system.advantagesAr ? system.advantagesAr : system.advantages,
      disadvantages: locale === 'ar' && system.disadvantagesAr ? system.disadvantagesAr : system.disadvantages,
