...
export const useNutrientCalculator = () => {
  ...
  const speakInstructions = useCallback(async (text: string, options?: Partial<VoiceFeedback>) => {
  ...
  const calculateNutrients = useCallback((recipe: NutrientRecipe, waterVolume: number): NutrientCalculation[] => {
    return recipe.nutrients.map(nutrient => {
      ...
      return {
        ...
        instructions,
        voiceInstructions,
        instructionsAr: isRTL ? instructions : t('nutrient.instructions.add', {
          amount: displayAmount.toFixed(2),
          unit: displayUnit,
          nutrient: nutrient.name,
          volume: waterVolume,
        ...
        voiceInstructionsAr: isRTL ? voiceInstructions : t('nutrient.voice.add', {
          amount: Math.round(displayAmount * 100) / 100,
          unit: displayUnit,
          nutrient: nutrient.name
        }, { lng: 'ar' })
  ...
  const calculateRecipe = useCallback(async (recipe: NutrientRecipe) => {
  ...
  }, [calculateNutrients, generateCalculationResult, state.waterVolume, state.voiceEnabled, speakInstructions, t]);

  // Save recipe functionality
  const saveRecipe = useCallback(async (recipe: NutrientRecipe, customName?: string) => {
    try {
  ...
  const syncOfflineData = useCallback(async () => {
    ...
    try {
      ...
      for (const recipe of pendingRecipes) {
        await supabase
          .from('saved_nutrient_recipes')
          .upsert([{ ...recipe, syncStatus: 'synced' }]);
      }
  ...
  const api = useMemo(() => ({
    ...
    selectRecipe: (recipe: NutrientRecipe) => setState(prev => ({ 
      ...prev, 
      selectedRecipe: recipe 
    })),
    
    setWaterVolume: (volume: number) => setState(prev => ({ 
      ...prev, 
      waterVolume: volume 
    })),
