...
export const useLightingCalculator = () => {
  ...
  const calculatePPFD = useCallback(
    (ledSpec: LEDSpecification, distance: number, dimming: number = 100): number => {
      ...
      const calculatedPPFD =
        (referencePPFD * Math.pow(referenceDistance, 2)) / Math.pow(distanceInches, 2);
  ...
  const calculatePowerCost = useCallback(
    (wattage: number, hoursPerDay: number, days: number = 30) => {
  ...
  const calculateCoverage = useCallback(
    (ledSpec: LEDSpecification, distance: number): number => {
    ...
    (
      ...
      if (ppfd < optimalPPFD.min) {
        recommendations.push({
          ...
          descriptionAr: t(
            'lighting.recommendations.ppfd_low_desc',
            {
              current: Math.round(ppfd),
              optimal: optimalPPFD.optimal,
            },
        ...
        recommendations.push({
          ...
          descriptionAr: t(
            'lighting.recommendations.ppfd_high_desc',
            {
              current: Math.round(ppfd),
              optimal: optimalPPFD.optimal,
            },
      ...
      if (dli < optimalDLI.min) {
        recommendations.push({
          ...
          descriptionAr: t(
            'lighting.recommendations.dli_low_desc',
            {
              current: dli.toFixed(1),
              optimal: optimalDLI.optimal,
            },
      ...
      if (stageSpectrum) {
        ...
        if (Math.abs(currentSpectrum.red - stageSpectrum.red) > 10) {
          recommendations.push({
            ...
            descriptionAr: t(
              'lighting.recommendations.spectrum_red_desc',
              {
                stage: stageId,
                recommended: stageSpectrum.red,
              },
  ...
  const createTimer = useCallback(
    async (setupId: string, photoperiod: PhotoperiodSchedule): Promise<LightingTimer> => {
  ...
  const calculateNextStateChange = (photoperiod: PhotoperiodSchedule): Date => {
    ...
    const sunrise = new Date(
      today.getTime() + sunriseHour * 60 * 60 * 1000 + sunriseMinute * 60 * 1000
    );
  ...
  const scheduleTimerNotifications = async (timer: LightingTimer) => {
    ...
    const beforeTrigger = new Date(
      trigger.getTime() - timer.notifications.beforeMinutes * 60 * 1000
    );
  ...
  const saveLightingSetup = useCallback(
    async (
      name: string,
      ...
      try {
        ...
        if (!state.isOffline) {
          await supabase.from('lighting_setups').insert([setup]);
        }
  ...
  const syncOfflineData = useCallback(async () => {
    ...
    try {
      ...
      for (const setup of pendingSetups) {
        await supabase.from('lighting_setups').upsert([{ ...setup, syncStatus: 'synced' }]);
      }
  ...
  const api = useMemo(
    () => ({
      ...
      selectLED: (led: LEDSpecification) =>
        setState(prev => ({
          ...prev,
          selectedLED: led,
        })),
      ...
      setPhotoperiod: (photoperiod: PhotoperiodSchedule) =>
        setState(prev => ({
          ...prev,
          photoperiod,
        })),
